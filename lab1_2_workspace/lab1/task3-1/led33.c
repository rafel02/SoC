...
static char ledName[7] = "led";
static unsigned int blinkingPeriod = 1000; // blink period in ms
 
enum modes { OFF, ON, BLINK };      // enum for LED modes
static enum modes mode = ON;        // default mode on

module_param(gpioLED,unsigned int,0);
MODULE_PARM_DESC(gpioLED,"this is the LED pin");
module_param(gpioSW, unsigned int, 0);
MODULE_PARM_DESC(gpioSW,"this is the LED pin");
...
static struct kobj_attribute period_attr = __ATTR(blinkingPeriod, 0660, period_show, period_store);
...
static int blink(void *arg) {
    while(!kthread_should_stop()) {     //returns true when kthread_stop() is called
        ...
        msleep(blinkingPeriod/2);       // sleep half of the period (cycle of on+off is one period)
    }
    return 0;
}
...
static int __init gpio_init(void) {
	...
	printk(KERN_INFO "LED Pin = %d SWITCH Pin = %d\n", gpioLED, gpioSW);
	
    // let's create a entry in sysfs (/sys/kernel/lab/myled)
    sprintf(ledName, "led");
    led_kobj = kobject_create_and_add("task", kernel_kobj); // kernel_kobj points to /sys/kernel
    if(!led_kobj) {
    ...
    printk(KERN_ALERT "GPIO_LED: sysfs group created successfully\n");
...
static irq_handler_t gpio_irq_handler(unsigned int irq, void *dev_id, struct pt_regs *regs) {
		printk(KERN_INFO "GPIO_SWITCH 69 callled\n");
        ledState = gpio_get_value(gpioSW); // invert led state
